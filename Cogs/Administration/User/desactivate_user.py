...
def desactivate_user_cogs(database):
    ...
    if verify_login(database) and verify_login(database) != 'desactivated':
        ...
        user_permission = database.select("""SELECT desactivate_account, admin FROM cantina_administration.permission WHERE user_token = %s""", (request.cookies.get('token')), 1)[0]
